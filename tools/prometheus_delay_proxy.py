...
class Handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        try:
            req = Request(target, headers={"User-Agent": "delay-proxy"})
            with urlopen(req, timeout=20) as resp:
                body = resp.read()
                self.send_response(resp.status)
                for k, v in resp.getheaders():
                    lk = k.lower()
                    if lk not in ("transfer-encoding", "connection", "keep-alive", "proxy-authenticate", "proxy-authorization", "te", "trailers", "upgrade"):
